...
const Profile = ({user}) => {
    ...
    useEffect(() => {
        setFormData({name: user.name, email: user.email, photo:user.photo, gender: user.gender, carModel: user.carModel})
    },[user])
            ...
            <form onSubmit={submitHandler}>
                ...
                <div className="mt-7">
                    <button
                        ...
                        {loading ? <HashLoader size={25} color="#ffffff"/> : 'Zaktualizuj swoje dane'}
                    </button>
                </div>
            </form>
        </div>
